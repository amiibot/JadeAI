...
- **自托管部署**：通过 Docker 一行命令部署（`docker run -d -p 3000:3000 -e AUTH_SECRET=<密钥> -v jadeai-data:/app/data csania/jadeai:latest`），并确保 `/app/data/local-auth-users.json` 存在；也可以本地 clone 源码开发。
