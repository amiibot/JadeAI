...
- **G2**：支持 **App 品牌主题**一键切换（默认 `jade` / 可选 `boss`），通过 `data-brand` 属性 + CSS variable 覆写。
...
```css
:root {
  /* Default: jade brand (light mode) */
  --brand: oklch(0.62 0.17 162);          /* #059669 emerald-600, AA on white */
  --brand-foreground: oklch(1 0 0);        /* white */
  --brand-hover: oklch(0.55 0.17 162);     /* darker */
  --brand-muted: oklch(0.95 0.05 162);     /* tag bg / selected bg */
  --brand-ring: oklch(0.62 0.17 162 / 40%);
}

.dark {
  --brand: oklch(0.78 0.15 162);           /* #34d399, brighter for dark */
...
.dark[data-brand="boss"],
[data-brand="boss"].dark {
  --brand: oklch(0.83 0.16 168);           /* #1FE0AC */
...
- 应用方式：在 `theme-provider.tsx` 同层增加 `BrandProvider`，挂载时给 `<html>` 添加 `data-brand="boss"`（jade 是默认，无需属性）
