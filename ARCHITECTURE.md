...
| 状态管理 | Zustand |
| 数据库 ORM | Drizzle ORM |
| 认证 | NextAuth.js (Auth.js v5) + Credentials |
...
│   │   ├── auth/                       # 认证层
│   │   │   ├── index.ts               # 认证入口
│   │   │   ├── config.ts             # NextAuth credentials 配置
│   │   │   ├── family-users.ts       # 本地家庭账号配置解析
│   │   │   ├── password.ts           # 本地密码 hash / 校验
│   │   │   └── helpers.ts            # 认证工具函数
│   │   │
│   │   ├── ai/                         # AI 层
│   │   │   ├── index.ts
...
│                         │                                           │
│  ┌──────────────────────┴────────────────────────────┐              │
│  │ next-intl (国际化) + NextAuth Session Provider    │              │
│  └───────────────────────────────────────────────────┘              │
└────────────────────────────┬────────────────────────────────────────┘
                             │ HTTP / SSE
┌────────────────────────────┴────────────────────────────────────────┐
...
当前认证固定为本地家庭账号登录，使用 NextAuth Credentials + 本地数据库用户映射。
...
└──────────────────┬──────────────────────────┘
         ...
         │                              │
         │ resolveUser():               │
         │ - 从认证会话读取 session.user │
         │ - 返回稳定 user.id / authType │
         └──────────────────────────────┘
```
...
| `email` | VARCHAR(255) | NULLABLE, UNIQUE | 邮箱（可选） |
| `name` | VARCHAR(255) | NULLABLE | 显示名称 |
| `avatar_url` | TEXT | NULLABLE | 头像 URL |
| `username` | VARCHAR(255) | NULLABLE, UNIQUE | 本地登录用户名 |
| `auth_type` | ENUM('local') | NOT NULL | 认证方式 |
...
// --- 枚举 ---
authTypeEnum = pgEnum('auth_type', ['local'])
...
```
┌──────┐     ┌──────────────┐     ┌──────────────────────┐     ┌──────────┐
│客户端│     │  登录页面     │     │ NextAuth Credentials │     │  数据库  │
└──┬───┘     └──────┬───────┘     └──────────┬───────────┘     └────┬─────┘
   │                │                                  │                  │
   │ 输入用户名密码 │                                  │                  │
   ├───────────────▶│                                  │                  │
